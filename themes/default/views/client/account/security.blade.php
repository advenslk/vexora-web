    ...
    <div class="px-2 flex flex-col gap-4">
        ...
        <div class="bg-background-secondary rounded-lg p-4">
            ...
            <div class="flex flex-row items-center justify-between py-2 border-b border-base/50">
                <div>
                    <p class="text-sm text-base/80">{{ $session->ip_address }} -
                        {{ $session->last_activity->diffForHumans() }}</p>
                    <p class="text-sm text-base/55">{{ $session->formatted_device }}</p>
                </div>
        ...
        <div class="bg-background-secondary rounded-lg p-4">
            ...
            @if ($twoFactorEnabled)
            <p class="text-sm text-base/75">{{ __('account.two_factor_authentication_enabled') }}</p>
            ...
            @else
            <p class="text-sm text-base/75">{{ __('account.two_factor_authentication_description') }}</p>
            ...
            <x-modal :title="__('account.two_factor_authentication_enable')" open="true">
                <p class="text-base/75">{{ __('account.two_factor_authentication_enable_description') }}</p>
                <div class="flex flex-col items-center mt-4">
                    <img src="{{ $twoFactorData['image'] }}" alt="QR code" class="w-64 h-64" />
                    <p class="mt-2 text-center text-sm text-base/55">
                ...
                <x-slot name="closeTrigger">
                    <button @click="document.location.reload()" class="text-base/70 hover:text-base">
