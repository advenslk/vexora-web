...
<fieldset class="flex flex-col w-full {{ $divClass ?? '' }}">
    ...
    <select id="{{ $id ?? $name }}" {{ $multiple ? 'multiple' : '' }} {{ $attributes->except(['options', 'id', 'name', 'multiple'])->merge(['class' => 'block w-full rounded-md border border-neutral/80 bg-background-secondary/80 px-3 py-2.5 text-sm text-base outline-none transition-all duration-200 ease-out form-select focus:border-primary focus:ring-1 focus:ring-primary disabled:bg-background-secondary/50 disabled:cursor-not-allowed']) }} name="{{ $name }}{{ $multiple ? '[]' : '' }}">
