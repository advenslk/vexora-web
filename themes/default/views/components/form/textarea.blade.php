...
<fieldset class="flex flex-col w-full {{ $divClass ?? '' }}">
    ...
    <textarea type="{{ $type ?? 'text' }}" id="{{ $id ?? $name }}" name="{{ $name }}"
        class="block w-full rounded-md border border-neutral/80 bg-background-secondary/80 text-sm text-base transition-all duration-200 ease-out placeholder:text-base/35 focus:border-primary focus:ring-1 focus:ring-primary disabled:bg-background-secondary/50 disabled:cursor-not-allowed {{ $class ?? '' }} @if ($type !== 'color') px-3 py-2.5 @endif"
